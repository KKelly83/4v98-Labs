...
function getSurveyQuestion($i) {
    ...
    <form action='' method='get' role='form'>
        <div class='form-group'>
            <label for='answer' id='question'>" . htmlspecialchars($questions[$i]) . "</label>
            <input type='text' name='answer$i' id='answer' class='form-control'/>
        </div>
        <input type='button' value='Next' class='form-control' onclick='nextQ();'/>
    </form>";
    return $form;
}
...
if (!isset($_SESSION['OnQuestion'])) {
    ...
    if (isset($_GET['answer' . $_SESSION['OnQuestion']])) {
...
if ($_SESSION['OnQuestion'] <= 2) {
    echo "<h1 id='questionNumber'>Question #" . ($_SESSION['OnQuestion'] + 1) . "</h1>";
    echo getSurveyQuestion($_SESSION['OnQuestion']);
} else {
    echo "<h1>Survey Completed!</h1>";
}
